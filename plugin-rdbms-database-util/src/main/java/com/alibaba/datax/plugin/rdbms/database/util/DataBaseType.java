...
public enum DataBaseType {
    ...
    public String buildFindTableListSql(String database) {
        String result = null;
        String suffix = null;
        switch (this) {
            case MySql:
            case DRDS:
                result = "SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES  WHERE 1=1 and TABLE_TYPE='BASE TABLE'  AND TABLE_SCHEMA = '"+database+"'";
                break;
            case Oracle:
                result = "SELECT B.TABLE_NAME FROM ALL_TAB_COMMENTS B  INNER JOIN  ALL_TABLES A ON A.TABLE_NAME = B.TABLE_NAME AND UPPER(A.OWNER) = '"+database.toUpperCase()+"'";
                break;
            case SQLServer:
                result = String.format("SELECT  A.[name] FROM [%s].[sys].[tables] A LEFT JOIN [%s].[sys].[extended_properties] C ON C.[major_id] = A.[object_id] ",database,database);
                break;
            case DB2:
                result = "SELECT TABNAME FROM SYSCAT.TABLES WHERE OWNER != 'SYSIBM' AND TABSCHEMA ='"+database+"'";
                break;
            case PostgreSQL:
                result = String.format("select pgc.relname from  pg_class pgc where 1=1 and pgc.relkind ='r' and pgn.nspname = '%s'",database);
                break;
            case ClickHouse:
                break;
            case RDBMS:
