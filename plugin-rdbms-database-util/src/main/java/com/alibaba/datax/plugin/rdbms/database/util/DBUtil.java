package com.alibaba.datax.plugin.rdbms.database.util;
...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        //ob10的处理
        if (url.startsWith(com.alibaba.datax.plugin.rdbms.database.writer.Constant.OB10_SPLIT_STRING)) {
            String[] ss = url.split(com.alibaba.datax.plugin.rdbms.database.writer.Constant.OB10_SPLIT_STRING_PATTERN);
            if (ss.length != 3) {
    ...
    public static List<String> getTables(DataBaseType dataBaseType,
                                               String jdbcUrl, String user, String pass, String database) {
        Connection conn = getConnection(dataBaseType, jdbcUrl, user, pass);
        return getTablesByConn(dataBaseType, conn, database );
    }

    public static List<String> getTablesByConn(DataBaseType dataBaseType,Connection conn,String database){
        ...
        try {
            statement = conn.createStatement();
            queryTablesSql = dataBaseType.buildFindTableListSql(database);
            if (StringUtils.isBlank(queryTablesSql)){
                throw DataXException.asDataXException(DBUtilErrorCode.REQUIRED_VALUE, String.format("整库同步暂不支持[%s]数据库,敬请关注.",dataBaseType.toString()));
            }
            rs = statement.executeQuery(queryTablesSql);
            ...
            throw RdbmsException.asQueryException(dataBaseType,e,queryTablesSql,database,null);
        } finally {
            DBUtil.closeDBResources(rs, statement, conn);
        }
        return tables;
